...
async function main() {
    ...
    _print("========== 质押 =========")
    _print(`总共有  : ${totalBPTAmount} BPT issued by ${rewardTokenTicker} DAI Balancer Pool.`);
    _print(`总共有  : ${totalStakedBPTAmount} BPT staked in ${rewardTokenTicker}'s BPT staking pool.`);
    ...
    _print(`每小时估算: ${toFixed(YFFIWeeklyEstimate / (7 * 24), 4)} ${rewardTokenTicker} = ${toDollar((YFFIWeeklyEstimate / (7 * 24)) * YFFIPrice)} (out of total ${toFixed(weekly_reward / (7 * 24), 2)} ${rewardTokenTicker})`)
    _print(`每日估算  : ${toFixed(YFFIWeeklyEstimate / 7, 4)} ${rewardTokenTicker} = ${toDollar((YFFIWeeklyEstimate / 7) * YFFIPrice)} (out of total  ${toFixed(weekly_reward / 7, 2)} ${rewardTokenTicker})`)
    _print(`每周估算  : ${toFixed(YFFIWeeklyEstimate, 4)} ${rewardTokenTicker} = ${toDollar(YFFIWeeklyEstimate * YFFIPrice)} (out of total ${weekly_reward} ${rewardTokenTicker})`)
