...
async function main() {
    ...
    _print("========== 质押 =========")
    _print(`总共有   : ${totalBPTAmount} UNI-V2 issued by MTA-WETH Uniswap Pool.`);
    _print(`           = ${toDollar(totalBPTAmount * BPTPrice)}`);
    _print(`总共有   : ${totalStakedBPTAmount} UNI-V2 staked.`);
    _print(`           = ${toDollar(totalStakedBPTAmount * BPTPrice)}\n`);
    _print(`你在质押 : ${yourBPTAmount} UNI-V2 (${toFixed(yourBPTAmount * 100 / totalStakedBPTAmount, 3)}% of the pool)`);
